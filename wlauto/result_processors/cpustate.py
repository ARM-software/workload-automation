...
from wlauto import ResultProcessor, instrumentation, Parameter
from wlauto.exceptions import ConfigError, ResultProcessorError

def addifnonzero(l, tt):
    if tt[1] != 0:
        l.append(tt)

def addifnonzerof(d, f, tt):
    if tt[1] != 0:
        d[f].append(tt)
...
class PowerTrace(object):
    ...
    """
    Base class representing a power trace line
    """
...
class IdleStartTraceCpuIdle(IdleStartTrace):
    ...
    def __init__(self, _match, _time):
        ...
        assert(
            state != -1 and state != 4294967295), "matched an idle end trace"
...
class IdleEndTraceCpuIdle(IdleEndTrace):
    ...
    def __init__(self, _match, _time, _cpu=None):
        ...
        """
        # parse _comment string for taskname, pid and priority
        IdleEndTrace.__init__(
            self,
            _match,
            _time,
            _cpu)  # __match format is the same
...
class FreqTrace(PowerTrace):
    ...
    """
    rgx = re.compile(
        '\s*power_frequency:\s+type=(\d+)\s+state=(\d+)\s+cpu_id=(\d+)')
...
class CpuStateProc(object):
    ...
    def __init__(self):
        ...
        self.cpuset = collections.defaultdict(
    ...
    def initialize(self, context, logger, absentstate=2, clusterstate=2, initfreq=None, tracefile='trace.txt'):  # pylint: disable=R0912
        ...
        if not initfreq:
            self.initfreq = [-1 for _ in self.clusterlist]
        else:
    ...
    def plotcores(self, trace, plotcores, name): # pylint: disable=R0914
        ...
        for c in plotcores:
            ...
            for t in ctrace:
                if isinstance(t, IdleEndTrace) and laststate != -1:
                    if laststate == 0:
                        laststate = 0.5  # make WFI show up
            ...
            for item in pl:
                ...
                yy = [0, item[2]]
                if xx[1]:
                    ax.broken_barh([xx], yy, alpha=0.8)
        fig.savefig(name)

    def debugdmp(self, trace, fname):
        # env variable PDEBUG needs to be set to get this stuff
        dbgpath = os.environ.get('CPUSTATE_DEBUG')
        plcores = os.environ.get('CPUSTATE_PLOTCORES')
        if plcores:
    ...
    def pass_final_do_self_trace(self, trace):
        ...
        for item in trace:
            ...
            if isinstance(item, FreqTrace):
                (clix, _) = self.getclusterix(item.cpu)
    ...
    def pass3_common_cluster_states(self, trace): #pylint: disable=R0914, R0912
        '''
        ...
        for item in trace:
            ...
            if isinstance(
                    item,
                    IdleStartTrace) and item.state >= self.clusterstate:
                ...
                if numcpus != len(laststate[clix]):
                    ...
                    for k in range(cnt, len(trace)):
                        ...
                        if isinstance(
                                nextwake,
                                IdleEndTrace) and nextwake.cpu in self.clusterlist[clix]:
                            break
                    if nextwake == -1:
                        ...
                        if c != item.cpu:
                            ...
                            newtrace.append(
                                IdleStartTrace(
                                    ...
                                    noncommonstate,
                                    True))
                            # using item.wakefromcommon state like this will be
                            # if we introduce several levels of cluster depth
                            # (but not in Ares and TC2)
                            laststate[wclix][wcpix] = IdleStartTrace(
                                None,
                                item.timestamp,
                                c,
                                item.wakefromcommon)
...
#         # take out non-common entries and change state into non-common state
        for item in newtrace:
            if isinstance(
                    item,
                    IdleStartTrace) and item.state >= self.clusterstate and not item.common:
    ...
    def pass2_initfreqs_notrace_cpus(self, maxcpu, trace):
        ...
        for c in range(maxcpu + 1):
            tracesforc = sum([self.cpuset[c][t]
                              for t in self.cpuset[c].keys()])
            if tracesforc == 0:  # no traces for c at all
                self.logger.warning('No traces observed for CPU %d' % (c))
                # if this is the case, add a trace begining and end of idle
                # start and stop
                trace.insert(
                    0,
                    IdleStartTrace(
        ...
        for clix in range(len(self.clusterlist)):
            ...
            newitemf = FreqTrace(
                ...
                self.firsttime,
                c,
                self.initfreq[clix])  # this is broadcast to other CPU below
    ...
    def parserunning(self):
        ...
        """
        self.running = collections.defaultdict(
        ...
        for k, ttraces in self.trace_dictionary.items():
            ...
            (clix, _) = self.getclusterix(k)
            lastfreq = self.initfreq[clix]
            running = True
            ...
            for item in ttraces:
                if isinstance(item, IdleStartTrace):
                    ...
                    addifnonzerof(self.running[k], lastfreq, a)
                ...
                if isinstance(item, FreqTrace):
                    ...
                    if running:
                        ...
                        # sometimes we get a new freq trace when still idle
                        newfreq = item.freq
            if running:
    ...
    def parseidlestates(self):
        ...
        """
        self.idlestates = collections.defaultdict(
            lambda: collections.defaultdict(list))
        ...
        for k in self.trace_dictionary.keys():
            ...
            for item in trace:
                if isinstance(item, IdleStartTrace):
                    ...
                    assert(
                        laststateitem == None), "last item %r item %r" % (laststateitem, item)
                    laststateitem = item
    ...
    def idle_parse_common(self, cpulist, state=None):
        """
        ...
        if state != None only include records where the requested idle
        state == state in other words if state is not in list supplied then
        cpu is considered as running
        """
        ret = collections.defaultdict(list)

        # traces are fixed so all cores start active
        laststate = [None for _ in cpulist]
        ...
        for item in self.trace:
            ...
            if isinstance(
                    item,
                    IdleStartTrace) and (
                    state == None or item.state in state):
                assert laststate[idx] == None
                # cpu idx is going idle
                nactivecpus = sum([s == None for s in laststate])
                delta = item.timestamp - lastentrytime
                if delta > 0:
                    ret[nactivecpus].append((lastentrytime, delta))
            ...
            if isinstance(
                    item,
                    IdleEndTrace) and (
                    state == None or laststate[idx] in state):
                assert laststate[idx] != None
                nactivecpus = sum([s == None for s in laststate])
                # cpu idx is waking
                delta = item.timestamp - lastentrytime
    ...
    def trace_first_pass(self, fh):
        ...
        for line in fh:
            ...
            if mm:
                timestamp = int(mm.group(1)) * 1000000 + int(mm.group(2))
                if self.realfirsttime < 0:
                    self.realfirsttime = timestamp
            if 'EVENTS DROPPED' in line:
                self.logger.warning(
                    'There are dropped events we will skip traces')
    ...
    def parse(self, context):
        """
        read file and create trace objects and lists
        """
        ...
        trace = self.pass3_common_cluster_states(trace)
        self.debugdmp(trace, "3_fixup_cluster_commons")
        self.pass_final_do_self_trace(trace)
    ...
    def addwarning(self, fh):
        if self.droppedtraces:
            ...
            if self.realfirsttime != -1:
                ...
                missingtime = '%d us %f%% of total' % (
                    realtotaltime - tracedtime, 100.0 * float(realtotaltime - tracedtime) / realtotaltime)
    ...
    def produce_report(self):
        ...
        self.parallel_report(
            nclusters,
            fh)  # calling with index set to max+1 generates report for all cores
        fh.close()
...
class CpuState(ResultProcessor):
    ...
    # classesoldformat = [IdleStartTrace, IdleEndTrace, FreqTrace] if ever we
    # need to go back to 3.4
    classesnewformat = [
        IdleEndTraceCpuIdle,
        IdleStartTraceCpuIdle,
        FreqTraceCpu]
    ...
    def validate(self):
        if not instrumentation.instrument_is_installed('trace-cmd'):
            raise ConfigError(
                '"dvfs" works only if "trace_cmd" in enabled in instrumentation')

    def initialize(self, context):  # pylint: disable=R0912
        self.cpustateproc.initialize(
            context,
            self.logger,
            self.absentstate,
            self.clusterstate,
...
class simplelogger(object):
    ...
    def warning(self, s):   # pylint: disable=R0201
        sys.stderr.write('warning: %s\n' % (s))

    def debug(self, s):  # pylint: disable=R0201
...
class fakedevice(object):
    ...
    def __init__(self):
        self.core_names = list()
        self.core_clusters = list()
...
def run():
    parser = argparse.ArgumentParser(
        description='Produce CPU power activity data from power trace.')
    parser.add_argument('infile', help='filename of trace file (text format)')
    parser.add_argument(
        '--clusterstate',
        default=2,
        help='first cluster gating state')
    parser.add_argument(
        '--absentstate',
        default=2,
        help='state to use as starting state if a cpu was idle from start of trace and for cpus where there is no trace at all')
    parser.add_argument(
        '--corenames',
        required=True,
        default="",
        help='comma separated core names eg A7, A7, A15, A15')
    parser.add_argument(
        '--coreclusters',
        required=True,
        default="",
        help='comma separated list of cluster id per core eg 0, 0, 1, 1')
    parser.add_argument(
        '--initfreq',
        required=True,
        default="",
        help='comma separated list of initial frequencies eg 10, 10')
    ...
    if len(fk.device.core_names) != len(fk.device.core_clusters):
        sys.stderr.write(
            'error: corenames (%d) and corecluster (%d) parameters must be same length \n' %
            (len(
                fk.device.core_names), len(
                fk.device.core_clusters)))
        parser.print_help()
        sys.exit(2)
    ...
    cpustateproc.initialize(
        fk, sl, int(
            args.absentstate), int(
            args.clusterstate), initfreq, args.infile)
